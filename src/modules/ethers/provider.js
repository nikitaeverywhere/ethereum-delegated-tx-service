...
for (const address of ethereumGlobalConfig.rpcProviders) {
  console.log(`Blockchain provider: registered RPC API for ${ ethereumGlobalConfig.networkName } to ${ address }`);
...
if (ethereumGlobalConfig.etherscanProviderApiKey) {
  ...
  console.log(`Blockchain provider: registered Etherscan fallback RPC API for ${ ethereumGlobalConfig.networkName }`);
}
